...
- The current catalog title must match a SubsPlease title after punctuation/case normalization, **or match a verified alternate title for the same anime**. When direct search finds no exact show, AniList is queried for English/romaji/native titles and synonyms. One uniquely identified anime with an exact title/season match is required; the first merely similar search result is never accepted. For example, `The Ogre's Bride` can resolve to `Oni no Hanayome`.
- Upload prefixes such as `S01EP11` are removed from the title search, while the season remains significant. `Season 2`, `S02`, and `2nd Season` suffixes are equivalent; cross-season episode-number guessing is not used.
- Quality must match exactly (`480p`, `540p`, `720p`, or `1080p`). Older SubsPlease 540p batches are **not** offered for 480p files. Unknown or unavailable qualities get no button.
- Episodes and combined ranges must match exactly. `01–13` does not point to `01–12`, and episode 1 does not point to a batch. A whole-season upload without a known episode end can use a single unambiguous batch starting at episode 1 for that season/quality; competing batch ranges are not guessed.
- Matching always uses the current title/category/files. Renames and category changes invalidate the old identity. The archive response carries a revision, and the card is re-read after lookup, so a change made while a request was running cannot attach old links to new files. Non-anime cards are never searched.
