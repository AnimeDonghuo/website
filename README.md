...
- **one key is one quota, so a pool is what a bulk publish needs.** `IMGBB_API_KEYS` takes up to 20 keys (comma, space, or newline separated — `IMGBB_API_KEY_2` … `IMGBB_API_KEY_20` work too for hosts that keep one secret per line, and `IMGBB_API_KEY` stays the first key). Uploads go **round-robin** across the pool, each key paced on its own clock, so ten keys carry ten uploads per window without any single quota seeing a burst;
...
- when **every** key is cooling and the remaining wait is longer than `IMGBB_KEY_COOLDOWN_MS`, the poster is deferred to the retry queue below instead of parking the publish for an hour;
