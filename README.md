...
| `/players SB-0123ABCDEF` | List every attached player with its server name, provider URL, and Remove buttons |
...
The title in front of the target ID is a safety check, not a search: it only has to agree with the card that ID points at (a shorter form of the same name is fine), and a clearly different name stops the merge instead of moving files onto the wrong card. A file that cannot be placed in the episode index is named in the result, because a merge that appeared to lose episodes is otherwise impossible to tell apart from one that worked. A mistyped ID (`Sb -29292`, `SB-112233445`) is reported as not being a Post ID rather than silently ignored.
...
Bare `/players` opens a short list of recent post IDs to choose from, and `/players SB-…` lists that card's players with **Remove** buttons.
