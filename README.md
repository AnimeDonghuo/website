...
- **Every listing is a page, not a cap.** A browse category, a search result, and a collection are fetched sixty cards at a time and the grid ends with `Showing 120 of 412 releases · page 2 of 7` and a **Load 60 more** button, so a catalog that grows past one screen keeps every older post reachable instead of quietly losing everything after the first hundred. The count comes from the store rather than from the page, and the next page is *appended*, so nothing the reader already scrolled past disappears under them. A failed page keeps the cards on screen and offers the button again
- **Genres and categories in the three-line menu.** The mobile drawer carries a `Genres & categories` row that opens the whole shelf list inside the menu — every category with the number of releases in it, and every genre tag the catalog uses, each one a link to a paged listing of exactly that. The two shelf rows are laid out like the menu rows above them (a hairline, a muted icon, a chevron) under an `EXPLORE` label, and the panel they open nests under them rather than appearing as two boxes glued to the foot of a list; the drawer paints itself from `--menu-*` tokens, so Day mode gets a light menu instead of a black slab. On a listing, the count line and its three shelf buttons (`Collections`, `Genres`, `Categories`) are one row of equal-width pills — the category control only appears where the hero's own rail is hidden, and it expands the rail in place instead of floating a popover over the grid it is meant to change. A category nobody has filled yet is dimmed and dashed rather than daring the tap. **18+ is never in that list**: the endpoint that answers it skips the adult category inside the query, so an unconfirmed visitor cannot reach restricted cards by guessing a genre URL either. The same list has its own page at `/genres` for a wide screen, and a genre is a shelf cut across categories (`/browse?genre=Action` holds movies, anime, and donghua alike)
- **Collections group a franchise without anyone maintaining a list.** A card whose title is a numbered, `Chapter`/`Part`/`Vol.`, colon-subtitled, or year-suffixed entry of another title joins that group on its own — `Iron Man`, `Iron Man 2`, `Iron Man 3`, and `Iron Man: The Final Cut` all appear on one **Iron Man** collection page. Nothing is stored that can go stale: the group is derived from the title at publish time, follows a `/title` correction, and is rebuilt for older cards by `/repair go`. `/collections` lists the groups, each card links its own group from its detail page, and a group of one — or a group whose only "second title" is a duplicate of the first — is never shown. See [collections](#collections-franchises-and-the-delete-that-follows-through)
...
A collection is a group of releases a viewer wants in order, and the site builds the groups from the titles it already has:

```text
/collection SB-0123ABCDEF, SB-1122334455 Marvel Saga
/collection SB-0123ABCDEF none
```

- **the normal case needs no command.** A card is put in its group when it is published (`/done`, `/batch`, auto-publish), moved when `/title` renames it, and grouped retroactively for everything published earlier by `/repair go`, which reports the cards that joined a group (`joins the "Iron Man" collection`) in its usual list;
- **`/collection <ID> Name`** overrides that for the two cases a title cannot express: several cards that belong together under a name no title spells (`/collection SB-1, SB-2, SB-3 MCU Phase One` — one message, every named post), and a card that should not be grouped at all (`/collection SB-1 none`, also `clear`, `remove`, `unset`, `off`). A hand-set name is *kept*: a later `/title` correction never recomputes it away, and a hand-cleared card stays out even when its new title looks like a sequel;
- **a group of one is not a collection.** A card always carries its own base name, but nothing is published as a franchise until two *different* titles share it, so a single film, or an accidental duplicate card, never becomes a fake series;
- **18+ is never collected.** Adult cards carry no group, are not counted in a group, and never appear on a collection page — a collection is a public surface like the homepage rail, and the exclusion happens in the query rather than after the page is cut;
- pasting upload junk into a title does not fool the grouping: `-KyoGo`, `DUAL`, `1080p`, `🔊`, a trailing year, and a signed group handle are all stripped before the group is named, so `Deadpool 2 DUAL -KyoGo mkv 🔊 #` sits with `Deadpool`.
...
| `GET /api/content?collection=iron-man` | Every part of a franchise, paged |
| `GET /api/content?page=3&limit=60` | Any listing's third page; `offset`-style paging is done with `page` |
| `GET /api/content?q=title` | Search titles, episode numbers, genres, or languages — every term in `q` has to match the same card |
| `GET /api/collections` | The franchise groups the catalog contains — only those holding at least two different titles, with their counts, categories, poster strip, and latest release date |
| `GET /api/collections/:key?page=1&limit=60` | One group's details and its cards, paged (`404` for a key no group holds) |
